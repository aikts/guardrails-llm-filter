...
Единственная группа переменных **без** префикса `GUARDRAILS_`: их читает SDK
OpenTelemetry и его экспортёры (`internal/tracing`), а не `internal/config`,
поэтому имена — стандартные из спецификации OTel. Трассировка **выключена**,
пока не задан эндпоинт: без него ни экспортёр, ни провайдер не создаются, и
открытие спана на горячем пути стоит одну проверку на nil.

Сквозной `traceparent` работает и с выключенной трассировкой: W3C-пропагатор
ставится всегда, поэтому входящий заголовок подхватывается, а исходящий запрос
к провайдеру его несёт — трасса вызывающего не рвётся на этом хопе.

| Переменная | По умолчанию | Примечания |
|---|---|---|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | — | OTLP-эндпоинт коллектора; **пусто = трассировка выключена**. Схема в значении решает транспортную безопасность: `http://` — без TLS, `https://` — с TLS |
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | — | то же, только для трасс; имеет приоритет |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | `grpc` | `grpc` (порт коллектора 4317) \| `http/protobuf` (4318). Неизвестное значение — отказ старта, а не молчаливый откат к другому транспорту |
| `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL` | — | то же, только для трасс; имеет приоритет |
| `OTEL_SERVICE_NAME` | `guardrails-llm-filter` | `service.name` в трассе |
| `OTEL_RESOURCE_ATTRIBUTES` | — | дополнительные атрибуты ресурса (`key=value,...`). Имя сервиса задаётся только через `OTEL_SERVICE_NAME`: `service.name` отсюда перекрывается дефолтом выше |
| `OTEL_TRACES_SAMPLER` | `parentbased_always_on` | сэмплер SDK, например `parentbased_traceidratio`; решение вызывающего уважается |
| `OTEL_TRACES_SAMPLER_ARG` | — | аргумент сэмплера, например `0.1` |

Остальные стандартные переменные экспортёров OTLP (`..._HEADERS`, `..._TIMEOUT`,
`..._COMPRESSION`, `..._CERTIFICATE` и т. д.) читают сами экспортёры — сервис их
не перехватывает.

**Что попадает в спаны.** Сервис видит сырые чувствительные значения, заголовок
`Authorization` клиента и ответ модели — ничего из этого в спаны не уходит.
Только метаданные: сопоставленный маршрут (из конфигурации, а не сырой путь
запроса), диалект, режим, размеры тел и текстов, счётчики, идентификаторы
сработавших правил и типов данных — ровно тот словарь, что уже отдают метрики и
заголовки ответа. Ошибочные спаны несут фиксированное описание, а не текст
ошибки: он оборачивает исходящий URL, а query-строка в нём — клиентская.
Инвариант закреплён тестом `TestTracing_SpansCarryNoSensitiveData`.

Подключение коллектора и дерево спанов — в
[../monitoring/](../monitoring/README.md#4-подключить-трассировку-opentelemetry).
