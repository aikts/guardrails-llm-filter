...
- **SSE** (`Content-Type: text/event-stream` от upstream либо стриминговый запрос): кадры
  ...
  отдают, уходит одним кадром. В `/v1/responses` так же обрабатываются и `reasoning_text`,
  и reasoning summary: дельты, `.done`, `part.done` и снимки в `output_item.done` и
  `response.completed`. LiteLLM отдаёт `reasoning_content` чат-модели именно как summary.
  Поле дельты или событие, которое процессор не знает, пересылается как есть, поэтому
  новый носитель текста модели нужно добавлять в процессор.
