...
По SIGTERM/SIGINT сервис останавливается в две фазы:

1. **Дренаж** (`GUARDRAILS_SHUTDOWN_DRAIN_PERIOD`, по умолчанию `0s` — фазы нет).
   `/readyz` отдаёт 503, data-plane перестаёт держать keep-alive (idle-соединения
   закрываются, каждый ответ закрывает своё), но листенер открыт и обслуживает запросы.
   За это время балансировщик (kube-proxy, service mesh) выводит экземпляр из ротации:
   клиенты, которые до него ещё доходят, обслуживаются, а не получают отказ в соединении,
   а клиенты с пулом соединений переподключаются к другим репликам.
2. **Остановка** (`GUARDRAILS_SHUTDOWN_TIMEOUT`, по умолчанию `10s`). Листенеры
   закрываются, запросы в полёте — стримы и нестриминговые ответы, ещё ждущие upstream, —
   получают столько времени на завершение. Что не успело, обрывается при выходе
   процесса, а upstream мог уже выполнить (и тарифицировать) такой запрос.

Если через сервис идут долгие генерации, задайте дренаж на время обновления эндпоинтов
(порядка 10 с) и бюджет не меньше самого долгого ответа upstream, а
`terminationGracePeriodSeconds` пода — больше их суммы (дефолт Kubernetes — 30 с).
