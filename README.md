...
[![Release](https://img.shields.io/badge/release-v0.1.2-2ea44f)](CHANGELOG.md)
...
Это форк [`cloud-ru-tech/guardrails-llm-filter`](https://github.com/cloud-ru-tech/guardrails-llm-filter)
с доработками, которые нужны нам раньше, чем появятся в апстриме, и со сборкой своего образа.

**Форк — одна ветка `kts`: апстрим плюс наши изменения.** `main` форка повторяет апстрим и
своих коммитов не несёт. В `kts` через PR вливаются наши изменения и новые теги апстрима,
с неё же режутся выпуски; история `kts` не переписывается — по ней собраны выпущенные
образы. Каждое изменение — своя ветка и PR в `kts`: самостоятельный коммит с тестами и
сообщением в стиле апстрима (Conventional Commits на английском). Список ниже — все наши
изменения, которых нет в апстриме, и PR с новым изменением дополняет его. Их коммиты
показывает `git log --reverse --no-merges <тег апстрима>..kts`, где тег — последний
влитый в `kts` (сейчас `v0.1.2.1`); коммиты изменений, которые апстрим позже принял, там
остаются, но из списка убираются. Изменения поверх `v0.1.2.1`:
...
4. `chore(deps): bump Go to 1.26.6 and grpc to 1.83.2` — security-бамп по находкам
   `govulncheck`, только для форка. Нужен, только пока апстрим держит версии ниже; когда
   апстрим поднимет Go и grpc до этих версий или выше, мерж его тега конфликтует в
   `go.mod`/`go.sum` — это и есть сигнал: берётся версия апстрима, пункт уходит из списка.
...
8. `docs: list the whole fork stack` — правка этого раздела, только для форка.
9. `feat(observability): optional OpenTelemetry tracing for the data plane` — спаны
   маскирования, вызова апстрима и демаскирования (включая SSE) по OTLP и сквозной
   `traceparent`; выключено, пока не задан `OTEL_EXPORTER_OTLP_ENDPOINT`. В спаны
   уходят только метаданные запроса — ни тел, ни найденных значений, ни заголовков.
...
12. `fix(sseproc): demask reasoning summaries in Responses API streams` — события
    `response.reasoning_summary_*` в стриме `/v1/responses` пересылались как есть, а LiteLLM
    отдаёт в них `reasoning_content` чат-модели. Теперь они демаскируются, как
    `reasoning_text`. Заодно дополняет этот раздел.

**Кандидаты в апстрим** — 1, 5, 6, 7, 9, 11 и 12: в них нет ничего специфичного для форка, PR в
...
```sh
git remote add upstream https://github.com/cloud-ru-tech/guardrails-llm-filter.git  # один раз
git fetch upstream --tags
git switch -c merge/<новый тег> origin/kts
git merge <новый тег>
make test lint
git push -u origin merge/<новый тег>   # и PR в kts
```

Такой PR вливается только merge-коммитом, не squash и не rebase: иначе тег апстрима не
станет предком `kts`. Перед мержем проверьте, какие пункты списка апстрим принял (или
сделал равноценно): в их конфликтах берётся версия апстрима, а пункты уходят из списка.

**Теги и образ.** Выпуск форка — тег `v<тег апстрима>-kts.<n>` на коммите ветки `kts`, где
тег апстрима — последний влитый в `kts`, например `v0.1.2.1-kts.1`; `n` растёт с каждым
выпуском на той же базе и начинается с `1` после мержа нового тега апстрима. Выпуски
`v0.1.2.1-kts.1` и `v0.1.2.1-kts.2` собраны ещё с прежней ветки `kts/v0.1.2.1`, которую
заменила `kts`; их коммиты — в истории `kts`. Пуш тега выпуска запускает
[`.github/workflows/release.yml`](.github/workflows/release.yml): GitHub Actions форка
собирает корневой `Dockerfile` и публикует `ghcr.io/aikts/guardrails-llm-filter:<тег>` —
ровно с этим тегом, без `latest`; `GET /v1/version` сообщает тот же тег. Теги апстрима
(`v*` без суффикса) форк не выпускает. CI гоняется и на пушах в `kts`. В форке
GitHub Actions по умолчанию выключены (включаются на вкладке *Actions*), а пакет в ghcr
при первой публикации создаётся приватным — видимость меняется в настройках пакета.

---

Клиенты обращаются к прокси вместо провайдера. По пути к модели ~265 встроенных
...
- 🛡️ **~265 встроенных правил**: учётные данные, API-ключи, access-токены, IP-адреса,
