...
**Форк — стек патч-коммитов поверх тега апстрима, а не отдельная линия разработки.**
...
git switch -c kts/<новый тег> <новый тег>
git cherry-pick <старый тег>..kts/<старый тег>   # по одному, по порядку
make test lint
```

Перед переносом коммита проверьте, не принят ли он (или равноценное изменение) в апстрим:
такой коммит не переносится, и стек становится короче. Ветки `kts/<старый тег>` не
удаляются и не переписываются — по ним собраны выпущенные образы.

**Теги и образ.** Выпуск форка — тег `v<тег апстрима>-kts.<n>` на коммите ветки
`kts/<тег апстрима>`, например `v0.1.2.1-kts.1`; `n` растёт с каждым выпуском на той же
базе и начинается с `1` на новой. Пуш такого тега запускает
